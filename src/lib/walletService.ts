...
export async function executeWalletTransaction(params: {
  ...
  console.log(
    `[Transaction Initiated] User: ${userId} | Type: ${type} | Amount: ₹${numAmount}`,
  )
  ...
  try {
    ...
    if (walletList && walletList.length > 0) {
      ...
      if (newWallet && newWallet.length > 0) {
        ...
        console.log(
          "[Wallet Created] Initialized zero-balance wallet for user:",
          userId,
        )
      } else {
        walletRecord = {
    ...
    if (
      isDebit &&
      !walletManagedExternally &&
      balanceAfter < 0 &&
      type !== "Admin Debit"
    ) {
      console.warn(
        `[Transaction Rejected] Insufficient wallet balance. Current: ₹${balanceBefore}, Requested: ₹${numAmount}`,
      )
      return {
        success: false,
        transactionId: "",
    ...
    if (txnErr) {
      console.error(
        "[Transaction Failed] Database transaction insert error:",
        txnErr,
      )
      return {
        success: false,
    ...
    console.log(
      `[Transaction Created] ID: ${txnId} | Before: ₹${balanceBefore} | After: ₹${balanceAfter}`,
    )

    // 3. Update wallet balance (skip for types managed externally by withdrawalService)
    if (!walletManagedExternally) {
      const updatedTotalEarned = !isDebit
        ? Number(walletRecord.total_earned || 0) + numAmount
        : Number(walletRecord.total_earned || 0)
      ...
      if (walletUpdateErr) {
        console.error(
          "[Wallet Update Failed] Reverting transaction status:",
          walletUpdateErr,
        )
        ...
        console.log(
          "[Rollback Executed] Marked transaction as Failed due to wallet update failure.",
        )
        return {
          success: false,
          transactionId: txnId,
      ...
      console.log(
        `[Wallet Updated] Balance updated to ₹${balanceAfter} for user: ${userId}`,
      )
    }
    ...
    return {
      ...
      transactionId: "",
      balanceAfter: 0,
      message: err.message || "Transaction exception occurred.",
    }
  }
}
  ...
  try {
    ...
    const actualBalance =
      walletData && walletData.length > 0
        ? Number(walletData[0].available_balance || 0)
        : 0
