...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  try {
    const [
      ...
      supabase.from('user_profiles').select('id, subscription_status, account_status'),
      supabase.from('payments').select('id, amount, status, created_at'),
      supabase.from('withdrawals').select('id, amount, status'),
      supabase.from('referral_stats').select('successful_referrals, referral_earnings'),
    ...
    const premiumCount = profiles.filter(p => p.subscription_status === 'PREMIUM').length
    const blockedCount = profiles.filter(p => p.account_status === 'BLOCKED').length

    const successPays = pays.filter(p => (p.status || '').toUpperCase() === 'CAPTURED' || (p.status || '').toUpperCase() === 'SUCCESS')
    const failedPays = pays.filter(p => (p.status || '').toUpperCase() === 'FAILED')
    const pendingPays = pays.filter(p => (p.status || '').toUpperCase() === 'PENDING')

    const totalRevenue = successPays.reduce((sum, p) => sum + Number(p.amount || 0), 0)
    const pendingWds = wds.filter(w => w.status === 'PENDING')
    const approvedWds = wds.filter(w => w.status === 'COMPLETED' || w.status === 'APPROVED')
    const rejectedWds = wds.filter(w => w.status === 'REJECTED')

    const totalSuccessfulReferrals = refs.reduce((sum, r) => sum + Number(r.successful_referrals || 0), 0)
    const totalReferralRewards = refs.reduce((sum, r) => sum + Number(r.referral_earnings || 0), 0)
    const totalWalletBalance = ws.reduce((sum, w) => sum + Number(w.available_balance || 0) + Number(w.pending_balance || 0), 0)
    ...
    return {
      ...
      pendingWithdrawalsAmount: pendingWds.reduce((sum, w) => sum + Number(w.amount || 0), 0),
      approvedWithdrawalsAmount: approvedWds.reduce((sum, w) => sum + Number(w.amount || 0), 0),
    ...
    return {
      totalUsers: 0, activeUsers: 0, premiumUsers: 0, freeUsers: 0,
      ...
      approvedWithdrawalsAmount: 0, rejectedWithdrawalsCount: 0,
      totalReferrals: 0, successfulReferrals: 0, totalReferralRewards: 0, totalWalletBalance: 0,
    }
  }
}
...
export async function getAdminUsers(): Promise<AdminUserRecord[]> {
  ...
  try {
    ...
    const walletMap = new Map<string, { available: number; total_earned: number }>()
    ...
    if (primaryUsers && primaryUsers.length > 0) {
      primaryUsers.forEach((u) => {
        ...
        const subscriptionStatus = (() => {
          const rawStatus = (profile?.subscription_status || u.subscription_status || 'FREE').toUpperCase()
        ...
        const subscriptionExpiry = profile?.subscription_expiry
          ...
          : (subscriptionStatus === 'PREMIUM' ? 'Active' : 'N/A')

        const accountStatus = (() => {
          const st = (u.status || profile?.account_status || 'active').toLowerCase()
          return (st === 'blocked' || st === 'BLOCKED') ? 'BLOCKED' as const : 'ACTIVE' as const
        })()
    ...
    if (dbPayments && dbPayments.length > 0) {
      return dbPayments.map((p) => {
        ...
        const status: PaymentTransaction['status'] =
          rawStatus === 'CAPTURED' || rawStatus === 'SUCCESS' ? 'SUCCESS'
    ...
    if (dbWithdrawals && dbWithdrawals.length > 0) {
      return dbWithdrawals.map((w) => {
        ...
        return {
          ...
          paymentDetails: w.payment_details || (w.upi_id ? `UPI: ${w.upi_id}` : 'N/A'),
          upiId: w.upi_id || '',
          status: (w.status || 'PENDING').toUpperCase() as WithdrawalRecord['status'],
          requestDate: w.requested_at || w.created_at
            ? new Date(w.requested_at || w.created_at).toLocaleString()
            : new Date().toLocaleString(),
          adminNotes: w.admin_notes || '',
          paymentRefId: w.transaction_id || '',
          reviewedAt: w.reviewed_at ? new Date(w.reviewed_at).toLocaleString() : undefined,
        }
      })
    }
  } catch (err) {
...
export async function getAdminReferrals(): Promise<ReferralRecord[]> {
  try {
    const { data: usersList } = await supabase.from('users').select('id, email, display_name, referral_code')
    const userMap = new Map<string, { email: string; name: string; code: string }>()
    if (usersList) {
      usersList.forEach((u) => {
        userMap.set(u.id, { email: u.email, name: u.display_name || u.email.split('@')[0], code: u.referral_code || '' })
      })
    }
    ...
    if (referrals && referrals.length > 0) {
      return referrals.map((r) => {
        ...
        const referrer = userMap.get(referrerId) || { email: 'N/A', name: 'Unknown', code: r.referral_code_used || '' }
        const referred = userMap.get(r.referred_user_id) || { email: 'N/A', name: 'Unknown', code: '' }
        return {
          id: r.id,
 ...
 */
export async function getAdminAuditLogs(limit = 100): Promise<AuditLogRecord[]> {
  try {
    ...
    if (logs && logs.length > 0) {
      return logs.map((l) => ({
        ...
        createdAt: l.created_at ? new Date(l.created_at).toLocaleString() : new Date().toLocaleString(),
      }))
    }
  } catch (err) {
...
export async function toggleUserStatus(userId: string, currentStatus: string) {
  ...
  try {
    await Promise.all([
      supabase.from('users').update({ status: newStatusLower, updated_at: new Date().toISOString() }).eq('id', userId),
      supabase.from('user_profiles').update({ account_status: newStatus, updated_at: new Date().toISOString() }).eq('id', userId),
    ])
  } catch (err) {
 ...
 */
export async function toggleUserSubscription(userId: string, email: string, grant: boolean) {
  const nowIso = new Date().toISOString()
  const expiry = grant ? new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString() : null
  const status = grant ? 'PREMIUM' : 'FREE'
  try {
    await Promise.all([
      supabase.from('user_profiles')
        .update({ subscription_status: status, subscription_expiry: expiry, updated_at: nowIso })
...
export function subscribeToAdminRealtimeUpdates(onUpdate: () => void) {
  const channel = supabase
    ...
    .on('postgres_changes', { event: '*', schema: 'public', table: 'wallets' }, () => onUpdate())
...
export function downloadCSV(filename: string, headers: string[], rows: (string | number)[]) {
  const csvContent =
    ...
    [headers.join(','), ...rows.map((e: any) => e.map((cell: any) => `"${String(cell).replace(/"/g, '""')}"`).join(','))].join('\n')
