...
export async function getRegisteredAdmins(): Promise<AdminUser[]> {
  try {
    const { data: admins, error } = await supabase
      .from('admins')
      .select('id, name, email, role, status, permissions, created_at, updated_at')
    ...
    if (admins && admins.length > 0) {
      return admins.map((a) => ({
        ...
        name: a.name || '',
        email: a.email,
        role: (a.role === 'SUPER_ADMIN' ? 'SUPER_ADMIN' : 'ADMIN') as 'SUPER_ADMIN' | 'ADMIN',
        status: (a.status === 'BLOCKED' ? 'BLOCKED' : 'ACTIVE') as 'ACTIVE' | 'BLOCKED',
        permissions: (() => {
          try {
            if (Array.isArray(a.permissions)) return a.permissions
            if (typeof a.permissions === 'string') return JSON.parse(a.permissions)
          } catch {}
          return []
        })(),
        lastLogin: a.updated_at ? new Date(a.updated_at).toLocaleString() : 'Never',
...
export async function createInitialSuperAdmin(
  ...
  const superAdminPermissions = [
    'manage_users', 'manage_payments', 'manage_subscriptions',
    'manage_referrals', 'approve_withdrawals', 'manage_cms',
  ...
  if (insertError) {
    throw new Error(`Failed to create Super Admin in database: ${insertError.message}`)
  }
  ...
  await logAdminAction(superAdmin.id, superAdmin.email, 'SUPER_ADMIN_SETUP',
...
export async function toggleAdminStatus(adminId: string): Promise<void> {
  try {
    ...
    const { data } = await supabase.from('admins').select('role, status').eq('id', adminId).single()
...
export async function deleteAdminAccount(adminId: string): Promise<void> {
  try {
    const { data } = await supabase.from('admins').select('role').eq('id', adminId).single()
...
export async function verifyAdminLogin(email: string, password: string): Promise<AdminUser> {
  ...
  const { data: adminRow, error: fetchError } = await supabase
    .from('admins')
    .select('id, name, email, role, status, permissions, password_hash, failed_attempts, locked_until, created_at')
    .eq('email', cleanEmail)
    .single()
  ...
  if (adminRow.locked_until && new Date(adminRow.locked_until) > new Date()) {
    const minutesLeft = Math.ceil((new Date(adminRow.locked_until).getTime() - Date.now()) / 60000)
    throw new Error(`Account locked. Try again in ${minutesLeft} minute(s).`)
  }

  // 3. Check status
  if (adminRow.status === 'BLOCKED') {
    throw new Error('This admin account has been suspended. Contact the Super Admin.')
  }
  ...
  const admin: AdminUser = {
    ...
    permissions: (() => {
      try {
        ...
        if (typeof adminRow.permissions === 'string') return JSON.parse(adminRow.permissions)
      } catch {}
      return []
    })(),
    lastLogin: nowIso,
    createdAt: adminRow.created_at ? new Date(adminRow.created_at).toISOString() : nowIso,
  }

  // 7. Create session
  _setAdminSession(admin)

  await logAdminAction(admin.id, admin.email, 'ADMIN_LOGIN', 'Logged into Admin Panel')
...
export async function logAdminAction(
  ...
  const deviceInfo = typeof navigator !== 'undefined' ? navigator.userAgent.slice(0, 120) : 'Server'
