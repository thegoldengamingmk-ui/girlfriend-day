...
export async function verifyAndProcessRazorpayPayment(params: {
...
}): Promise<{ success: boolean; message: string; alreadyProcessed?: boolean; paymentId?: string }> {
  const { userId = '', userEmail = '', amount, razorpayOrderId = '', razorpayPaymentId, razorpaySignature = '' } = params
  ...
  try {
    ...
    if (existingPayment && existingPayment.length > 0) {
      console.log('[Idempotency Guard] Payment already verified & processed previously:', razorpayPaymentId)
      return { success: true, message: 'Payment already processed.', alreadyProcessed: true, paymentId: razorpayPaymentId }
    }
    ...
    if (paymentErr) {
      ...
      console.log('[Payment Record Created] Saved payment to public.payments table:', razorpayPaymentId)
    }
    ...
    if (userId && userId.length > 20 && userId !== 'guest') {
      ...
      console.log('[Premium Activated] Successfully activated Premium subscription for user:', userId)
    }
    ...
    return { success: false, message: err.message || 'Payment processing exception.' }
  }
}
