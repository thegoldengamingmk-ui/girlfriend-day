 ...
 */
export async function createWithdrawalRequest(params: CreateWithdrawalParams): Promise<{
  success: boolean
  message: string
  withdrawalId?: string
  withdrawal?: any
}> {
  const { userId, amount, paymentMethod = 'UPI', upiId = 'N/A', paymentDetails = '' } = params
  ...
  if (numAmount < WITHDRAWAL_CONFIG.MIN_AMOUNT) {
    return { success: false, message: `Minimum withdrawal amount is ₹${WITHDRAWAL_CONFIG.MIN_AMOUNT}.` }
  }

  if (numAmount > WITHDRAWAL_CONFIG.MAX_AMOUNT) {
    return { success: false, message: `Maximum withdrawal limit per request is ₹${WITHDRAWAL_CONFIG.MAX_AMOUNT}.` }
  }
  ...
  try {
    ...
    if (walletErr) {
      console.error('[Withdrawal Request Failed] Wallet update error:', walletErr)
    ...
    if (wdInsertErr) {
      console.error('[Withdrawal Insert Error] Database insert warning:', wdInsertErr)
    }

    console.log(`[Withdrawal Requested] WD ID: ${wdId} | User: ${userId} | Amount: ₹${numAmount} | Available: ₹${newAvailable} | Pending: ₹${newPending}`)

    return {
      success: true,
      message: `Withdrawal request for ₹${numAmount} submitted successfully! It is under admin review.`,
      withdrawalId: wdId,
      withdrawal: inserted && inserted.length > 0 ? inserted[0] : withdrawalPayload,
    }
  } catch (err: any) {
...
export async function processAdminWithdrawalAction(params: {
  ...
  console.log(`[Admin Withdrawal Action] Action: ${action} | WD ID: ${withdrawalId} | Admin: ${adminEmail}`)
  ...
  try {
    ...
    const { data: list } = await supabase
      .from('withdrawals')
      .select('*')
      .or(`id.eq.${withdrawalId},withdrawal_id.eq.${withdrawalId},request_id.eq.${withdrawalId}`)
    ...
    if (wd.status === 'COMPLETED' || wd.status === 'REJECTED') {
      return { success: false, message: `Withdrawal has already been marked as ${wd.status}.` }
    }
    ...
    if (action === 'APPROVE') {
      ...
      console.log(`[Withdrawal Approved] WD ID: ${wd.withdrawal_id} | Paid: ₹${amount} to User: ${userId}`)
      return { success: true, message: `Withdrawal of ₹${amount} approved and marked as COMPLETED!` }
      ...
      await supabase
        ...
        .update({
          ...
          reviewed_at: nowIso,
          reviewed_by: adminEmail,
          admin_notes: adminNotes || 'Withdrawal rejected. Funds restored to available balance.',
          updated_at: nowIso,
        })
        .eq('id', wd.id)
      ...
      console.log(`[Withdrawal Rejected] WD ID: ${wd.withdrawal_id} | Restored: ₹${amount} to Available Balance`)
      return { success: true, message: `Withdrawal of ₹${amount} rejected. Funds restored to user wallet.` }
      ...
      return { success: true, message: 'Withdrawal status updated to PROCESSING.' }
    }

    return { success: false, message: 'Invalid action specified.' }
  } catch (err: any) {
    console.error('[Admin Withdrawal Action Error]:', err)
    return { success: false, message: err.message || 'Failed to process withdrawal action.' }
  }
}
