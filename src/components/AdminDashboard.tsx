...
function SectionTitle({ children }: { children: React.ReactNode }) {
  return (
    <h3 className="text-xs font-bold uppercase tracking-widest text-pink-300/70 mb-4">{children}</h3>
  )
}
...
function Th({ children, right }: { children: React.ReactNode; right?: boolean }) {
  return (
    <th className={`p-4 text-slate-400 uppercase tracking-wider font-semibold ${right ? 'text-right' : ''}`}>
      {children}
    </th>
  )
}
...
export function AdminDashboard({
  ...
  const [newAdminPermissions, setNewAdminPermissions] = useState<string[]>([
    'manage_users', 'manage_payments', 'approve_withdrawals',
  ])
  ...
  const handleToggleUserBlock = async (user: AdminUserRecord) => {
    ...
    await logAdminAction(admin.id, admin.email, 'TOGGLE_USER_STATUS',
      `${user.accountStatus === 'ACTIVE' ? 'Blocked' : 'Unblocked'} user: ${user.email}`)
    await reloadAllData()
    if (selectedUser?.id === user.id) {
      setSelectedUser(prev => prev ? { ...prev, accountStatus: prev.accountStatus === 'ACTIVE' ? 'BLOCKED' : 'ACTIVE' } : null)
    }
  }
  ...
  const handleToggleSubscription = async (user: AdminUserRecord) => {
    ...
    await logAdminAction(admin.id, admin.email, 'TOGGLE_SUBSCRIPTION',
  ...
  const handleWithdrawalAction = async (action: 'APPROVE' | 'REJECT') => {
    ...
    try {
      await processAdminWithdrawalAction({
        ...
        action,
        adminNotes: withdrawNotes || (action === 'APPROVE'
          ? `Approved. Ref: ${withdrawRefId || 'N/A'}`
          : 'Withdrawal rejected.'),
        adminEmail: admin.email,
      })
      await logAdminAction(admin.id, admin.email, `WITHDRAWAL_${action}`,
  ...
  const handleCreateStaffAdmin = async (e: React.FormEvent) => {
    ...
    setAdminFormMsg('')
    try {
      await createStaffAdmin(newAdminName, newAdminEmail, newAdminPass, newAdminRole, newAdminPermissions)
      await logAdminAction(admin.id, admin.email, 'CREATE_STAFF_ADMIN',
  ...
  const handleToggleAdminStatus = async (id: string) => {
    ...
    await logAdminAction(admin.id, admin.email, 'TOGGLE_ADMIN_STATUS', `Toggled admin status for ${id}`)
    await reloadAllData()
  }
  ...
  const handleDeleteAdmin = async (id: string, email: string) => {
    ...
    await logAdminAction(admin.id, admin.email, 'DELETE_ADMIN', `Deleted admin account: ${email}`)
    await reloadAllData()
  }
  ...
  const handleChangePassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logAdminAction(admin.id, admin.email, 'CHANGE_PASSWORD', 'Updated admin login password')
  ...
  const handleExportUsersCSV = () => {
    downloadCSV(`Users_Export_${Date.now()}.csv`,
      ['User ID', 'Name', 'Email', 'Mobile', 'Subscription', 'Wallet Balance', 'Referral Code', 'Referred Users', 'Earnings', 'Status', 'Signup Date', 'Last Login'],
      users.map((u) => [u.id, u.name, u.email, u.mobile, u.subscriptionStatus, u.walletBalance,
        u.referralCode, u.referredUsersCount, u.totalReferralEarnings, u.accountStatus, u.signupDate, u.lastLogin]) as any
    )
    logAdminAction(admin.id, admin.email, 'EXPORT_USERS_CSV', 'Exported Users dataset to CSV')
  }

  const handleExportPaymentsCSV = () => {
    downloadCSV(`Payments_Export_${Date.now()}.csv`,
      ['Transaction ID', 'User Name', 'Email', 'Amount', 'Gateway', 'Status', 'Date'],
      transactions.map((t) => [t.transactionId, t.userName, t.email, t.amount, t.gateway, t.status, t.date]) as any
    )
  ...
  const filteredUsers = users.filter((u) => {
    ...
    const matchFilter = userStatusFilter === 'ALL' ||
      u.subscriptionStatus === userStatusFilter || u.accountStatus === userStatusFilter
    return matchSearch && matchFilter
  })

  const filteredPayments = transactions.filter((t) => {
    const q = paymentSearch.toLowerCase()
    return !q || t.transactionId.toLowerCase().includes(q) ||
  ...
  const filteredReferrals = referrals.filter((r) => {
    ...
    return !q || r.referrerEmail.toLowerCase().includes(q) ||
      r.referredEmail.toLowerCase().includes(q) || r.referrerCode.toLowerCase().includes(q)
  })

  const filteredAuditLogs = auditLogs.filter((l) => {
    const q = auditSearch.toLowerCase()
    return !q || l.action.toLowerCase().includes(q) ||
      l.adminEmail.toLowerCase().includes(q) || l.description.toLowerCase().includes(q)
  })
  ...
  const tabs = [
    ...
    { id: 'payments', label: 'Payments', icon: '💳', badge: stats ? `₹${stats.totalRevenue}` : '' },
    { id: 'subscriptions', label: 'Subscriptions', icon: '💎', badge: premiumUsers.length },
    { id: 'referrals', label: 'Referrals', icon: '🎁', badge: referrals.length },
    { id: 'withdrawals', label: 'Withdrawals', icon: '💸',
      badge: withdrawals.filter(w => w.status === 'PENDING').length || undefined },
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex overflow-hidden font-sans">
      {/* ── SIDEBAR ── */}
      <aside className={`${isSidebarOpen ? 'w-64' : 'w-20'} transition-all duration-300 bg-slate-900/95 border-r border-slate-800 flex flex-col justify-between z-30 select-none shrink-0`}>
        <div>
          {/* Brand */}
            ...
            <div className="flex items-center gap-3 overflow-hidden">
              ...
              {isSidebarOpen && (
                <div className="truncate">
                  <h1 className="font-bold text-sm text-white leading-snug">Admin Portal</h1>
              ...
              >
                ...
                {isSidebarOpen && tab.badge !== undefined && tab.badge !== 0 && (
                  <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold shrink-0 ${
                    tab.id === 'withdrawals' ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-slate-800 text-pink-300 border border-slate-700'
                  }`}>
                    {tab.badge}
                  </span>
                )}
        ...
        <div className="sticky top-0 z-20 bg-slate-950/95 backdrop-blur-sm border-b border-slate-800 px-6 py-4 flex items-center justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-[11px] text-slate-400 mt-0.5">
              <span className={`w-2 h-2 rounded-full ${isRefreshing ? 'bg-amber-400 animate-pulse' : 'bg-emerald-400 animate-pulse'}`} />
              <span>{isRefreshing ? 'Refreshing...' : 'Live Supabase Data'}</span>
              <span>• <strong className="text-pink-300">{admin.role}</strong></span>
            </div>
          </div>
            ...
            <div className="space-y-8 animate-fade-up">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  <StatCard icon="👥" label="Total Users" value={stats?.totalUsers ?? users.length} sub="Registered accounts" />
                  <StatCard icon="✅" label="Active Users" value={stats?.activeUsers ?? '-'} color="#34d399" sub="Not blocked" />
                  <StatCard icon="💎" label="Premium" value={stats?.premiumUsers ?? premiumUsers.length} color="#a78bfa" sub="Active subscriptions" />
                  <StatCard icon="🆕" label="Today" value={stats?.newUsersToday ?? '-'} color="#60a5fa" sub="New registrations" />
                  <StatCard icon="📅" label="This Month" value={stats?.newUsersThisMonth ?? '-'} color="#f472b6" sub="New registrations" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  <StatCard icon="💰" label="Total Revenue" value={`₹${stats?.totalRevenue ?? '-'}`} color="#34d399" sub="Successful payments" />
                  <StatCard icon="✅" label="Successful" value={stats?.successfulPayments ?? '-'} color="#34d399" sub="Payment count" />
                  <StatCard icon="❌" label="Failed" value={stats?.failedPayments ?? '-'} color="#f87171" sub="Payment count" />
                  <StatCard icon="⏳" label="Pending" value={stats?.pendingPayments ?? '-'} color="#fbbf24" sub="Awaiting capture" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  <StatCard icon="⏳" label="Pending Count" value={stats?.pendingWithdrawalsCount ?? '-'} color="#fbbf24" sub="Awaiting approval" />
                  <StatCard icon="🔒" label="Pending Amount" value={`₹${stats?.pendingWithdrawalsAmount ?? '-'}`} color="#fbbf24" sub="Reserved balance" />
                  <StatCard icon="✅" label="Approved Amount" value={`₹${stats?.approvedWithdrawalsAmount ?? '-'}`} color="#34d399" sub="Total paid out" />
                  <StatCard icon="❌" label="Rejected" value={stats?.rejectedWithdrawalsCount ?? '-'} color="#f87171" sub="Rejected requests" />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
                  <StatCard icon="🎁" label="Total Referrals" value={stats?.totalReferrals ?? referrals.length} color="#c084fc" sub="Successful" />
                  <StatCard icon="💸" label="Referral Rewards" value={`₹${stats?.totalReferralRewards ?? '-'}`} color="#c084fc" sub="Total paid" />
                  <StatCard icon="🏦" label="Total Wallet" value={`₹${stats?.totalWalletBalance ?? '-'}`} color="#22d3ee" sub="All user balances" />
                  <StatCard icon="🛡️" label="Admin Role" value={admin.role} color="#fbbf24" sub={admin.email} />
                </div>
              </div>
              ...
              {withdrawals.filter(w => w.status === 'PENDING').length > 0 && (
                <div>
                  <SectionTitle>⚡ Pending Withdrawals — Action Required</SectionTitle>
                  <div className="space-y-2">
                    {withdrawals.filter(w => w.status === 'PENDING').slice(0, 5).map(w => (
                      <div key={w.id} className="flex items-center justify-between p-4 rounded-2xl bg-amber-500/5 border border-amber-500/20">
                        <div>
                          <p className="text-sm font-bold text-white">{w.userName} — <span className="text-emerald-400">₹{w.amount}</span></p>
                          <p className="text-[11px] text-slate-400">{w.email} • {w.paymentDetails}</p>
                        </div>
                        <button onClick={() => { setActiveTab('withdrawals'); setWithdrawalFilter('PENDING') }}
          ...
          {activeTab === 'users' && (
            <div className="space-y-4 animate-fade-up">
              <div className="flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between p-4 bg-slate-900/80 rounded-2xl border border-slate-800">
                ...
                <div className="flex gap-2 flex-wrap">
                  {['ALL', 'PREMIUM', 'FREE', 'ACTIVE', 'BLOCKED'].map((st) => (
                    <button key={st} onClick={() => setUserStatusFilter(st)}
              ...
              {filteredUsers.length === 0
                ? <EmptyState icon="👥" title="No Users Found" sub="No users match your search or filter." />
                : (
                  <AdminTable>
                    <THead>
                      <Th>User</Th>
                      ...
                      {filteredUsers.map((u) => (
                        <tr key={u.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-4">
                            ...
                            <div className="text-[10px] text-slate-500 mt-0.5">{u.signupDate}</div>
                          </td>
                          <td className="p-4"><StatusBadge status={u.subscriptionStatus} /></td>
                          <td className="p-4 font-bold text-emerald-400 font-mono">₹{u.walletBalance}</td>
                          <td className="p-4 font-mono font-bold text-pink-300 text-xs">{u.referralCode}</td>
                          <td className="p-4 text-slate-400 text-[11px]">{u.lastLogin}</td>
                          <td className="p-4"><StatusBadge status={u.accountStatus} /></td>
                          <td className="p-4 text-right">
                            <div className="flex gap-1.5 justify-end flex-wrap">
                              ...
                              <button onClick={() => handleToggleSubscription(u)}
                                className={`px-3 py-1.5 rounded-xl font-bold text-xs cursor-pointer ${
                                  u.subscriptionStatus === 'PREMIUM'
                                    ? 'bg-violet-500/20 text-violet-300 hover:bg-violet-500/30'
                                    : 'bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30'
                                }`}>
                                {u.subscriptionStatus === 'PREMIUM' ? 'Revoke' : 'Grant Premium'}
                              </button>
                              <button onClick={() => handleToggleUserBlock(u)}
                                className={`px-3 py-1.5 rounded-xl font-bold text-xs cursor-pointer ${
              ...
              <p className="text-xs text-slate-500 text-right">Showing {filteredUsers.length} of {users.length} users</p>
            </div>
          )}
            ...
            <div className="space-y-4 animate-fade-up">
              <div className="flex justify-between items-center p-4 bg-slate-900/80 rounded-2xl border border-slate-800">
                ...
                <div className="flex items-center gap-3">
                  <span className="text-xs text-emerald-400 font-semibold">● Razorpay Connected</span>
                  <span className="text-xs text-slate-400">{filteredPayments.length} records</span>
                </div>
              </div>

              {filteredPayments.length === 0
                ? <EmptyState icon="💳" title="No Payment Records" sub="No payment transactions found in the database." />
                : (
                  <AdminTable>
                    <THead>
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {filteredPayments.map((t) => (
                        <tr key={t.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-4 font-mono font-bold text-pink-300 text-[11px]">{t.transactionId}</td>
                          <td className="p-4">
                            <div className="font-bold text-white">{t.userName}</div>
                            <div className="text-slate-400 text-[11px] font-mono">{t.email}</div>
                          </td>
                          <td className="p-4 font-bold text-emerald-400 font-serif">₹{t.amount}</td>
                          <td className="p-4 text-slate-300 font-semibold">{t.gateway}</td>
                          <td className="p-4"><StatusBadge status={t.status} /></td>
                          <td className="p-4 text-slate-400 text-[11px]">{t.date}</td>
                        </tr>
                      ))}
                    </tbody>
                  </AdminTable>
          ...
          {activeTab === 'subscriptions' && (
            <div className="space-y-4 animate-fade-up">
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <StatCard icon="💎" label="Total Premium" value={premiumUsers.length} color="#a78bfa" sub="Active subscribers" />
                <StatCard icon="🆓" label="Free Users" value={users.filter(u => u.subscriptionStatus === 'FREE').length} color="#94a3b8" sub="No subscription" />
                <StatCard icon="⏰" label="Expired" value={users.filter(u => u.subscriptionStatus === 'EXPIRED').length} color="#f97316" sub="Lapsed subscriptions" />
                <StatCard icon="💰" label="Revenue" value={`₹${stats?.totalRevenue ?? '-'}`} color="#34d399" sub="From payments" />
              </div>

              {premiumUsers.length === 0
                ? <EmptyState icon="💎" title="No Premium Subscribers" sub="No users with active PREMIUM subscription." />
                : (
                  <AdminTable>
                    <THead>
                      <Th>User</Th>
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {premiumUsers.map((u) => (
                        <tr key={u.id} className="hover:bg-slate-800/40 transition-colors">
                          ...
                          <td className="p-4"><StatusBadge status={u.subscriptionStatus} /></td>
                          <td className="p-4 text-slate-300 text-xs">{u.subscriptionExpiry}</td>
                          <td className="p-4 font-bold text-emerald-400">₹{u.walletBalance}</td>
                          <td className="p-4 text-right">
                            <button onClick={() => handleToggleSubscription(u)}
          ...
          {activeTab === 'referrals' && (
            <div className="space-y-4 animate-fade-up">
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <StatCard icon="🎁" label="Total Referrals" value={referrals.length} color="#c084fc" sub="All time" />
                <StatCard icon="💸" label="Total Rewards" value={`₹${referrals.reduce((s, r) => s + r.commissionAmount, 0)}`} color="#c084fc" sub="Paid commissions" />
                <StatCard icon="✅" label="Approved" value={referrals.filter(r => r.status === 'APPROVED').length} color="#34d399" sub="Completed referrals" />
              </div>

              <div className="p-4 bg-slate-900/80 rounded-2xl border border-slate-800">
                <input type="text" value={referralSearch} onChange={(e) => setReferralSearch(e.target.value)}
              ...
              {filteredReferrals.length === 0
                ? <EmptyState icon="🎁" title="No Referral Activity" sub="No referral records found in the database." />
                : (
                  <AdminTable>
                    <THead>
                      <Th>Referrer</Th>
                      ...
                      {filteredReferrals.map((r) => (
                        <tr key={r.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-4">
                            ...
                            <div className="text-[11px] text-slate-400 font-mono">{r.referrerEmail}</div>
                          </td>
                          <td className="p-4 font-mono font-bold text-pink-300 text-xs">{r.referrerCode}</td>
                          <td className="p-4">
                            <div className="font-bold text-white">{r.referredName}</div>
                            <div className="text-[11px] text-slate-400 font-mono">{r.referredEmail}</div>
                          </td>
                          <td className="p-4 font-bold text-emerald-400">₹{r.commissionAmount}</td>
          ...
          {activeTab === 'withdrawals' && (
            <div className="space-y-4 animate-fade-up">
              <div className="flex gap-2 p-4 bg-slate-900/80 rounded-2xl border border-slate-800 flex-wrap">
                {['ALL', 'PENDING', 'COMPLETED', 'REJECTED', 'PROCESSING'].map((st) => (
                  <button key={st} onClick={() => setWithdrawalFilter(st)}
                    className={`px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer transition-colors ${
                      withdrawalFilter === st ? 'bg-pink-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'
                    }`}>
                    {st} {st !== 'ALL' ? `(${withdrawals.filter(w => w.status === st).length})` : `(${withdrawals.length})`}
                  </button>
                ))}
              </div>

              {filteredWithdrawals.length === 0
                ? <EmptyState icon="💸" title="No Withdrawal Requests" sub="No requests match the selected filter." />
                : (
                  <AdminTable>
                    <THead>
                      <Th>Request ID</Th>
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {filteredWithdrawals.map((w) => (
                        <tr key={w.id} className="hover:bg-slate-800/40 transition-colors">
                          <td className="p-4 font-mono font-bold text-amber-300 text-[11px]">{w.requestId}</td>
                          <td className="p-4">
                            <div className="font-bold text-white">{w.userName}</div>
                            <div className="text-[11px] text-slate-400 font-mono">{w.email}</div>
                          </td>
                          <td className="p-4 font-bold text-emerald-400 font-serif">₹{w.amount}</td>
                          <td className="p-4 text-slate-200 text-xs font-mono">{w.paymentDetails}</td>
                          <td className="p-4"><StatusBadge status={w.status} /></td>
                          <td className="p-4 text-slate-400 text-[11px]">{w.requestDate}</td>
                          <td className="p-4 text-right">
                            {((w.status as string) === 'PENDING' || (w.status as string) === 'PROCESSING') && (
                              <button onClick={() => { setActionWithdrawal(w); setWithdrawNotes(''); setWithdrawRefId('') }}
          ...
          {activeTab === 'admins' && admin.role === 'SUPER_ADMIN' && (
            <div className="space-y-6 animate-fade-up">
              <div className="flex justify-between items-center">
                <SectionTitle>Staff Admin Accounts ({adminAccounts.length})</SectionTitle>
                <button onClick={() => { setShowAddAdminModal(true); setAdminFormMsg('') }}
              ...
              {adminAccounts.length === 0
                ? <EmptyState icon="👑" title="No Admin Accounts" sub="No staff admins found. Create the first one." />
                : (
                  <AdminTable>
                    <THead>
                      <Th>Admin</Th>
                    ...
                    <tbody className="divide-y divide-slate-800/60">
                      {adminAccounts.map((a) => (
                        <tr key={a.id} className="hover:bg-slate-800/40 transition-colors">
                          ...
                          <td className="p-4"><StatusBadge status={a.role} /></td>
                          <td className="p-4"><StatusBadge status={a.status} /></td>
                          <td className="p-4 text-slate-400 text-[11px]">{a.createdAt ? new Date(a.createdAt).toLocaleDateString() : 'N/A'}</td>
                          <td className="p-4 text-right">
                            {a.role !== 'SUPER_ADMIN' && (
                              <div className="flex gap-1.5 justify-end">
                                <button onClick={() => handleToggleAdminStatus(a.id)}
                                  className={`px-3 py-1.5 rounded-xl font-bold text-xs cursor-pointer ${
                                    a.status === 'ACTIVE'
                            ...
                            {a.role === 'SUPER_ADMIN' && (
                              <span className="text-[11px] text-amber-300/60 italic">Protected</span>
                            )}
                          </td>
                        </tr>
                      ))}
            ...
            <div className="space-y-8 animate-fade-up">
              ...
              <div className="p-6 rounded-3xl bg-slate-900/80 border border-slate-800 max-w-md">
                ...
                <form onSubmit={handleChangePassword} className="space-y-4">
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">New Password</label>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Confirm Password</label>
                  ...
                  {passMsg && (
                    <p className={`text-xs font-semibold ${passMsg.startsWith('✅') ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {passMsg}
                    </p>
                  )}
                ...
                <div className="mt-4 p-3 rounded-xl bg-slate-950 border border-slate-800 text-[10px] text-slate-400 font-mono">
                  Password is hashed with SHA-256 + salt and stored in Supabase admins table.
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  <SectionTitle>📋 Admin Audit Logs ({auditLogs.length})</SectionTitle>
                ...
                {filteredAuditLogs.length === 0
                  ? <EmptyState icon="📋" title="No Audit Logs" sub="No admin activity logged yet." />
                  : (
                    <AdminTable>
                      <THead>
                        <Th>Action</Th>
                      ...
                      <tbody className="divide-y divide-slate-800/60">
                        {filteredAuditLogs.map((l) => (
                          <tr key={l.id} className="hover:bg-slate-800/40 transition-colors">
                            ...
                            <td className="p-4 text-slate-300 text-xs font-mono">{l.adminEmail}</td>
                            <td className="p-4 text-slate-400 text-xs max-w-xs truncate">{l.description}</td>
                            <td className="p-4 text-slate-500 text-[10px] max-w-[120px] truncate">{l.deviceInfo}</td>
                            <td className="p-4 text-slate-400 text-[11px]">{l.createdAt}</td>
                          </tr>
                        ))}
                      </tbody>
                    </AdminTable>
          ...
          >
            ...
            >
              ...
              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-xs">
                <h4 className="font-bold text-pink-300 uppercase tracking-wider text-[11px]">Account Details</h4>
                <div className="grid grid-cols-2 gap-2 text-slate-300">
                  <div>ID: <span className="font-mono text-white text-[10px]">{selectedUser.id.slice(0, 18)}...</span></div>
                  <div>Phone: <span className="text-white">{selectedUser.mobile}</span></div>
                  <div>Referral: <span className="font-mono font-bold text-amber-300">{selectedUser.referralCode}</span></div>
                  <div>Signup: <span className="text-white">{selectedUser.signupDate}</span></div>
                  <div>Wallet: <span className="text-emerald-400 font-bold">₹{selectedUser.walletBalance}</span></div>
                  <div>Earnings: <span className="text-emerald-300 font-bold">₹{selectedUser.totalReferralEarnings}</span></div>
                  <div>Referred Users: <span className="text-violet-300 font-bold">{selectedUser.referredUsersCount}</span></div>
                  <div>Last IP: <span className="text-white font-mono">{selectedUser.lastLoginIp}</span></div>
                </div>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-2 text-xs">
                <h4 className="font-bold text-pink-300 uppercase tracking-wider text-[11px]">🔐 Login History</h4>
                {isLoadingHistory
                      ...
                      <div className="space-y-1.5 max-h-40 overflow-y-auto">
                        {userLoginHistory.map((h) => (
                          <div key={h.id} className="p-2 rounded-xl bg-slate-900 border border-slate-800 flex justify-between items-center text-[11px]">
                            ...
                            <span className="px-2 py-0.5 rounded-md bg-emerald-500/20 text-emerald-300 text-[10px] font-bold">Login</span>
                          </div>
                        ))}
                      </div>
                    )}
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => handleToggleSubscription(selectedUser)}
                  ...
                  {selectedUser.subscriptionStatus === 'PREMIUM' ? 'Revoke Premium ⚡' : 'Grant Premium 💎'}
                </button>
                <button onClick={() => handleToggleUserBlock(selectedUser)}
                  className={`flex-1 py-2.5 rounded-2xl font-bold text-xs cursor-pointer ${
                    selectedUser.accountStatus === 'ACTIVE'
                      ? 'bg-rose-500/20 text-rose-300 border border-rose-500/30'
                      : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'
                  }`}>
                  {selectedUser.accountStatus === 'ACTIVE' ? 'Block Account ⛔' : 'Unblock ✅'}
                </button>
              </div>
              <button onClick={() => setSelectedUser(null)}
      ...
      <AnimatePresence>
        {actionWithdrawal && (
          <motion.div
            ...
            onClick={(e) => { if (e.target === e.currentTarget && !isProcessingWd) setActionWithdrawal(null) }}
          >
            <motion.div
            ...
            >
              <div>
                <h3 className="text-lg font-bold font-serif">Process Withdrawal</h3>
                <p className="text-xs text-slate-400 mt-0.5 font-mono">{actionWithdrawal.requestId}</p>
              </div>

              <div className="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-1">
                <p className="text-xs text-slate-300"><span className="text-slate-500">User:</span> {actionWithdrawal.userName}</p>
                <p className="text-xs text-slate-300"><span className="text-slate-500">Email:</span> {actionWithdrawal.email}</p>
                <p className="text-lg font-bold text-emerald-400 font-serif">₹{actionWithdrawal.amount}</p>
                <p className="text-xs font-mono text-pink-300">{actionWithdrawal.paymentDetails}</p>
              </div>

              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Payment Ref / UTR ID</label>
                <input type="text" value={withdrawRefId} onChange={(e) => setWithdrawRefId(e.target.value)}
              ...
              <div>
                <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Admin Notes</label>
              ...
              <div className="grid grid-cols-3 gap-2 pt-1">
                <button onClick={() => setActionWithdrawal(null)} disabled={isProcessingWd}
                ...
                <button onClick={() => handleWithdrawalAction('REJECT')} disabled={isProcessingWd}
                ...
                <button onClick={() => handleWithdrawalAction('APPROVE')} disabled={isProcessingWd}
      ...
      <AnimatePresence>
        {showAddAdminModal && admin.role === 'SUPER_ADMIN' && (
          <motion.div
            ...
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-[500] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md"
            onClick={(e) => { if (e.target === e.currentTarget) setShowAddAdminModal(false) }}
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 20 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
            ...
            >
              ...
              <form onSubmit={handleCreateStaffAdmin} className="space-y-3">
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Full Name</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Email</label>
                ...
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Password (min 12 chars)</label>
                  <input type="password" required value={newAdminPass} onChange={(e) => setNewAdminPass(e.target.value)}
                    placeholder="Strong password required"
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-700 text-xs text-white outline-none focus:border-pink-500"
                  />
                </div>
                <div>
                  <label className="block text-[11px] font-bold uppercase tracking-wider text-pink-300/80 mb-1">Role</label>
                ...
                {adminFormMsg && (
                  <p className="text-xs text-rose-400 font-semibold">⚠️ {adminFormMsg}</p>
                )}
