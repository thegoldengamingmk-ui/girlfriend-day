...
async function testReferralFlow() {
  ...
  if (users.length === 0) {
    console.log('No users in public.users table yet. Testing code lookup logic with fallback...')
  } else {
    const userA = users[0]
    ...
    // 2. Test Invalid Code
    const invalidResult = await validateAndApplyReferralCode(userA.id, userA.email, 'INVALID_CODE_123')
    ...
    const selfRefResult = await validateAndApplyReferralCode(userA.id, userA.email, userA.referral_code)
    console.log('Test Self-Referral Result:', selfRefResult)
    if (!selfRefResult.success && selfRefResult.message.includes('Self-referral')) {
...
async function validateAndApplyReferralCode(
  ...
  const { data: usersList } = await supabase.from('users').select('*').eq('referral_code', cleanCode)

  let referrer = usersList && usersList.length > 0 ? usersList[0] : null
  if (!referrer) {
    const { data: profileList } = await supabase.from('user_profiles').select('*').eq('referral_code', cleanCode)
