...
async function testWithdrawalWorkflow() {
  ...
  if (!wallet) {
    const { data: insertedW } = await supabase.from('wallets').insert([{ user_id: user.id, available_balance: 500 }]).select()
  ...
  const excessResult = validateWithdrawalRule(Number(wallet.available_balance), Number(wallet.available_balance) + 1000)
...
function validateWithdrawalRule(availableBalance: number, amount: number) {
  if (amount < 100) return { valid: false, message: 'Minimum withdrawal amount is ₹100.' }
  if (amount > availableBalance) return { valid: false, message: 'Insufficient wallet balance.' }
