...
async function testRazorpayVerification() {
  ...
  const invalidCalc = crypto
    .createHmac("sha256", secretKey)
    .update(expectedMsg)
    .digest("hex")
  if (invalidSignature !== invalidCalc) {
    console.log(
      "✓ PASS 1: Invalid payment signature correctly detected and rejected.",
    )
  } else {
  ...
  if (validSignature === invalidCalc) {
    console.log(
      "✓ PASS 2: Valid Razorpay HMAC-SHA256 signature verified successfully.",
    )
  } else {
  ...
  if (existing && existing.length > 0) {
    console.log(
      "✓ PASS 4: Idempotency guard verified. Duplicate payment events safely ignored.",
    )
  }
